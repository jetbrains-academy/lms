...
from core.timezone import now_local, UTC
...
from courses.tests.factories import (
    ...
    CourseTeacherFactory, LearningSpaceFactory, SemesterFactory, CourseProgramBindingFactory
)
...
def serialize_datetime(form: dict) -> dict:
    for k, v in form.items():
        if isinstance(v, datetime.date):
            form[k] = v.strftime('%d.%m.%Y')
        elif isinstance(v, datetime.time):
            form[k] = v.strftime('%H:%M')
    return form


@pytest.mark.django_db
def test_manager_for_student(settings, program_cub001, program_run_cub):
    student_profile = StudentProfileFactory(academic_program_enrollment=program_run_cub)
    student = student_profile.user
    teacher = TeacherFactory()
    cpb = CourseProgramBindingFactory(program=program_cub001, course__group_mode=CourseGroupModes.MANUAL)
    course = cpb.course
...
def test_view_course_class_create(client, lms_resolver, assert_login_redirect):
    ...
    form = factory.build(dict, FACTORY_CLASS=CourseClassFactory)
    form = serialize_datetime(form)
    venue = LearningSpaceFactory()
...
def test_course_class_form_available(client, curator, settings):
    ...
    course.save()
    response = client.get(course_class_add_url)
    assert smart_bytes("This course has ended") in response.content
    client.login(curator)
    response = client.get(course_class_add_url)
    assert smart_bytes("This course has ended") not in response.content
...
def test_course_class_detail_view_timezone(client, settings):
    curator = CuratorFactory(time_zone=ZoneInfo('Europe/Moscow'))
    client.login(curator)
    ...
    assert smart_bytes("Wednesday, 01 January 2020, 20:00–23:00") in response.content
    curator.time_zone = ZoneInfo('Asia/Novosibirsk')
    curator.save()
    response = client.get(course_class.get_absolute_url())
    assert smart_bytes("Thursday, 02 January 2020, 00:00–03:00") in response.content
