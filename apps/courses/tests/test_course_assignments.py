...
def test_course_assignment_update(client, assert_redirect):
    ...
    assert response.status_code == 200
    response = client.post(update_url, prefixed_form(form, "assignment"))
...
def get_assignment_course_models():
    ...
    sg0 = StudentGroup.objects.filter(course_id=course.pk)[0]
    sg1, sg2 = StudentGroupFactory.create_batch(2, course=course)
    url = course.get_create_assignment_url()
    return course, url, ct1, ct2, sg0, sg1, sg2


@pytest.mark.skip('Balanced mode is disabled, skipping all its tests')
@pytest.mark.django_db
...
def test_course_assignment_delete(client, assert_redirect):
    ...
    course_assignments_list = course.get_url_for_tab('assignments')
    assert_redirect(client.post(delete_url), course_assignments_list)
