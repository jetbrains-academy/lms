...
from courses.models import Assignment, Course, CourseClass, CourseTeacher, CourseProgramBinding
...
from courses.tests.factories import (
    ...
    MetaCourseFactory, SemesterFactory, CourseProgramBindingFactory
)
...
def test_course_class_starts_at_local():
    ...
    assert course_class.starts_at_local(UTC).strftime('%H:%M') == '20:20'
    assert course_class.starts_at_local(ZoneInfo('Asia/Novosibirsk')).strftime('%d %H:%M') == '02 03:20'
    ...
    course_class = CourseClass(date=ambiguous.date(),
                               ...
                               time_zone=eastern)
    assert course_class.starts_at_local() == ambiguous.replace(tzinfo=eastern)
...
def test_course_class_ends_at_local():
    ...
    assert course_class.ends_at_local(UTC).strftime('%H:%M') == '20:20'
    assert course_class.ends_at_local(ZoneInfo('Asia/Novosibirsk')).strftime('%d %H:%M') == '02 03:20'
    # Ambiguous dates will be resolved with is_dst=False
    eastern = ZoneInfo('US/Eastern')
    ambiguous = datetime.datetime(year=2002, month=10, day=27, hour=1, minute=30)
    course_class = CourseClass(date=ambiguous.date(),
                               ...
                               time_zone=eastern)
    assert course_class.ends_at_local() == ambiguous.replace(tzinfo=eastern)
...
@pytest.mark.django_db
def test_course_binding_constraints(program_cub001):
    course = CourseFactory()
    invitation = InvitationFactory()
    tomorrow = timezone.now() + datetime.timedelta(days=1)

    # We can bind a course to a program once...
    with transaction.atomic():
        binding = CourseProgramBinding(
            course=course,
    ...
    # We can bind a course and an invitation once...
    with transaction.atomic():
        binding = CourseProgramBinding(
            course=course,
    ...
    # We must bind a course to either a program or invitation...
    with transaction.atomic():
        binding = CourseProgramBinding(
            course=course,
            enrollment_end_date=tomorrow,
...
@pytest.mark.django_db
def test_course_binding_timezone_sync():
    tz_berlin = ZoneInfo('Europe/Berlin')
    tz_nicosia = ZoneInfo('Asia/Nicosia')
    end_date_berlin = datetime.datetime(2024, 9, 1, 0, 0, 0, tzinfo=tz_berlin)
    end_date_nicosia = datetime.datetime(2024, 9, 1, 0, 0, 0, tzinfo=tz_nicosia)
    binding = CourseProgramBindingFactory(enrollment_end_date=end_date_berlin, course__time_zone=tz_berlin)
    course = binding.course

    course.time_zone = tz_nicosia
    course.save()
    binding.refresh_from_db()
    assert binding.enrollment_end_date == end_date_nicosia
