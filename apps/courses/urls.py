...
RE_COURSE_URI = r"^" + re_semester_slug + r"/(?P<course_id>\d+)-(?P<course_slug>[-\w]+)/"
...
urlpatterns = [
    path("courses/", include([
        ...
        re_path(RE_COURSE_URI, include([
            ...
            path('faces/', include([
                path('', views.CourseStudentFacesView.as_view(), name='student_faces'),
                path('csv', views.CourseStudentFacesCSVView.as_view(), name='student_faces_csv'),
            ])),
        ])),
    ])),
