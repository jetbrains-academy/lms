...
class StudentFilter(FilterSet):
    ...
    universities = NumberInFilter(field_name='academic_program_enrollment__program__university_id')
    academic_programs = NumberInFilter(field_name='academic_program_enrollment__program_id')
    ...
    class Meta:
        ...
        fields = ("name", "universities", "academic_programs", "profile_types",
    ...
    def courses_filter(self, queryset, name, value):
        ...
        passing_grade = GradingSystems.get_passing_grade_expr('user__enrollment')
        queryset = queryset.annotate(
            courses_total=
            # Remove unsuccessful grades, then distinctly count by pk
            Count(Case(
                When(user__enrollment__grade__lt=passing_grade,
                     then=Value(None)),
