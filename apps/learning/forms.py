...
from learning.models import (
    AssignmentSubmissionTypes, StudentAssignment, StudentGroupTeacherBucket, StudentGroup
)
...
class CourseEnrollmentForm(forms.Form):
    reason = forms.CharField(
        label=_("Why did you choose this course?"),
        widget=forms.Textarea(),
        required=False,
    )

    def __init__(self, ask_reason, **kwargs):
        super().__init__(**kwargs)
        self.helper = FormHelper()
        layout = []
        if ask_reason:
            layout.append('reason')
        layout.append(Submit('enroll', 'Enroll'))
        self.helper.add_layout(Layout(*layout))


class CourseUnenrollmentForm(forms.Form):
    reason = forms.CharField(
        label=_("Why did you decide to disenroll from the course?"),
        widget=forms.Textarea(),
        required=False,
    )
    ...
    def __init__(self, ask_reason, **kwargs):
        ...
        if ask_reason:
            layout.append('reason')
        layout.append(Submit('disenroll', 'Disenroll'))
        self.helper.add_layout(Layout(*layout))
...
class StudentGroupTeacherBucketAdminForm(forms.ModelForm):
    ...
    def clean_groups(self):
        ...
        if buckets_with_same_groups.exists():
            ...
            raise ValidationError(
                mark_safe(f"Selected student groups overlap with "
