...
from learning.tests.factories import EnrollmentFactory, StudentAssignmentFactory, AssignmentCommentFactory
from users.services import get_student_profile
from users.tests.factories import StudentFactory, StudentProfileFactory, TeacherFactory, InvitedStudentFactory, \
    UserFactory
...
def test_view_student_assignment_as_teacher(client, assert_login_redirect):
    ...
    enrollment = EnrollmentFactory(course=course, grade=1)
...
def test_view_student_assignment_failed_course(client):
    teacher = TeacherFactory()
    course = CourseFactory(teachers=[teacher], completed_at=datetime.date.today())
    student_assignment = StudentAssignmentFactory(assignment__course=course,
...
@pytest.mark.django_db
def test_view_course_list(client, program_cub001, program_run_cub, program_nup001):
    ...
    CourseProgramBindingFactory(course__semester=s, program=program_cub001)
    CourseProgramBindingFactory(course__semester=s, program=program_nup001)
...
@pytest.mark.django_db
def test_view_assignment_list(client, student_type, program_cub001, program_run_cub):
    url = reverse('study:assignment_list')

    student = StudentFactory(student_profile__type=student_type)
...
@pytest.mark.django_db
def test_assignment_list_view_context_unenrolled_course(client, program_cub001, program_run_cub):
    """
    ...
    course = CourseProgramBindingFactory(course__semester=s, program=program_cub001).course
...
@pytest.mark.django_db
@pytest.mark.parametrize("learner_factory", [StudentFactory, InvitedStudentFactory])
def test_view_deadline_l10n_on_student_assignment_list_page(learner_factory,
    ...
    # Test `upcoming` block
    now_academic_year = get_current_term_pair().academic_year
    ...
    client.login(student_spb)
    response = client.get(url)
    assert response.context_data["tz_override"] == student_spb.time_zone
    year_part = formats.date_format(assignment_nsk.deadline_at_local(),
                                    FORMAT_DATE_PART)
    assert year_part == "01 января 2017"
    time_part = formats.date_format(
        assignment_nsk.deadline_at_local(tz=student_spb.time_zone),
        FORMAT_TIME_PART)
    assert time_part == "18:00"
    html = BeautifulSoup(response.content, "html.parser")
    assert any(year_part in s.text and time_part in s.text for s in
