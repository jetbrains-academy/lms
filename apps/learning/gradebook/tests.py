...
from learning.settings import (
    GradeTypes, StudentStatuses, EnrollmentGradeUpdateSource, GradingSystems
)
...
def test_save_gradebook(client, assert_redirect):
    ...
    for student in students:
        assert 4 == Enrollment.active.get(student=student, course=course).grade
...
def test_gradebook_data(settings):
    ...
    a1, a2, a3 = AssignmentFactory.create_batch(3, course=co, maximum_score=10)
    ...
    student_profile = get_student_profile(e5.student)
    student_profile.status = StudentStatuses.EXPELLED
    student_profile.save()
    a_new = AssignmentFactory(course=co, maximum_score=7)
    data = gradebook_data(co)
    s5_index = 4
    new_a_index = 3
...
def test_gradebook_view_form_invalid(client):
    ...
    assert form[final_grade_field_name].value() == 5
    form_data = {
        field_name: -5  # invalid value
    }
...
def test_gradebook_view_form_conflict(client):
    ...
    form = response.context_data['form']
    assert form[field_name].value() == 4
    assert form[final_grade_field_name].value() == 4
    sa.refresh_from_db()
    assert sa.score == 4
    e.refresh_from_db()
    assert e.grade == 4
    ...
    client.login(teacher2)
    form_data[final_grade_field_name] = 5
    response = client.post(co.get_gradebook_url(), form_data)
    assert response.status_code == 200
    assert response.context_data['form'].conflicts_on_last_save()
    messages = list(get_messages(response.wsgi_request))
    ...
    assert final_grade_field.value() == '5'
    ...
    sa.refresh_from_db()
    assert sa.score == 4
    form_data[final_grade_field_name] = 4
    response = client.post(co.get_gradebook_url(), form_data)
...
def test_gradebook_import_assignment_scores_from_csv_permissions(client):
    ...
    client.login(teacher2)
    url = reverse('teaching:gradebook_import_scores_by_enrollment_id', args=[co.pk])
    ...
    url = reverse('teaching:gradebook_import_scores_by_enrollment_id',
...
def generate_course(group_one_size: int = 5, group_two_size: int = 5):
    teacher = TeacherFactory()
    course = CourseFactory(teachers=[teacher],
                           group_mode=CourseGroupModes.MANUAL)
    group_one, group_two = StudentGroupFactory.create_batch(2, course=course)
    regular_student = StudentFactory.create_batch(group_one_size)
    invited_students = StudentFactory.create_batch(
        group_two_size, student_profile__type=StudentTypes.INVITED
    )
    # student profiles will be queried automatically based on the profile type
    for student in regular_student:
    ...
    for student in invited_students:
        EnrollmentFactory(
            ...
            course=course,
            student_group=group_two,
        )
    return teacher, course, regular_student, invited_students, group_one, group_two
...
def test_view_gradebook_filter_form_contains_all_student_groups(client):
    ...
    assert len(choices) == 5  # 3 groups + All + default
...
@pytest.mark.django_db
def test_gradebook_import_course_grades_permissions(client, lms_resolver,
    ...
    teaching_url = reverse('teaching:gradebook_import_course_grades_by_enrollment_id', args=[course.pk])
    staff_url = reverse('teaching:gradebook_import_course_grades_by_enrollment_id', args=[course.pk])
...
@pytest.mark.django_db
def test_gradebook_import_course_grades_wrong_headers(client):
    teacher = TeacherFactory()
    course = CourseFactory(teachers=[teacher])
    url = reverse('teaching:gradebook_import_course_grades_by_enrollment_id', args=[course.pk])
    e = EnrollmentFactory(course=course)
    assignment = AssignmentFactory.create(
        course=course,
        submission_type=AssignmentFormat.NO_SUBMIT,
    ...
    client.login(teacher)
    csv_data = force_bytes(f"""
wrong_id_header,header2,Final grade
{e.id},1,5
""".strip())
    ...
    assert f"Header 'id' not found" in str(messages[0])

    csv_data = force_bytes(f"""
id,header2,WrongGradeColumn
{e.id},1,5
""".strip())
    ...
    assert len(messages) == 1
    assert messages[0].level == messages_constants.ERROR
    assert f"Header 'final grade' not found" in str(messages[0])
...
def test_gradebook_import_course_grades_by_enrollment_id(client):
    ...
    import_csv_url = reverse('teaching:gradebook_import_course_grades_by_enrollment_id',
                             args=[course.pk])
    e1, e2, e3 = EnrollmentFactory.create_batch(
        3,
        course=course,
        course_program_binding__grading_system_num=GradingSystems.BINARY_PLUS_EXCELLENT
    )
    assignment = AssignmentFactory.create(
        course=course,
        submission_type=AssignmentFormat.NO_SUBMIT,
        maximum_score=500)
    csv_data = force_bytes(f"""
id,header2,Final grade
{e1.id},1,Excellent
0,3,1
,1,1
{e2.id},2,Re-credit
{e2.id},2,cadabra
incorrect-id,2,Excellent
{e3.id},3,Failed
{e3.id},3,Fail
{e3.id},3,Pass
{e3.id},3,1
""".strip())
    ...
    assert messages[1].level == messages_constants.ERROR
    assert '4 out of 7 rows with valid student IDs.' in str(messages[0])
    assert all(error_msg in str(messages[1]) for error_msg in [
        "Row 2: Student with ID '0' not found.",
        "Row 3: Student with ID '' not found.",
        f"Row 5: Grade 'cadabra' doesn't exist or isn't valid for this course's grading system. Student ID '{e2.id}'.",
        "Row 6: Student with ID 'incorrect-id' not found.",
        f"Row 7: Grade 'Failed' doesn't exist or isn't valid for this course's grading system. Student ID '{e3.id}'.",
        f"Row 10: Grade '1' doesn't exist or isn't valid for this course's grading system. Student ID '{e3.id}'."
    ])
    e1.refresh_from_db()
    e2.refresh_from_db()
    e3.refresh_from_db()
    ...
    assert e3.grade == GradeTypes.PASS
    ...
    assert log1.grade == GradeTypes.FAIL
    assert log2.grade == GradeTypes.PASS
    assert log2.entry_author == teacher


@pytest.mark.parametrize(
    'grading_system,valid_grades,invalid_grades',
    [
        (
            GradingSystems.BINARY,
            ('Not graded', 'Re-credit', 'Fail', 'Pass'),
            ('Good', 'Excellent', '5'),
        ),
        (
            GradingSystems.BINARY_PLUS_EXCELLENT,
            ('Not graded', 'Re-credit', 'Fail', 'Pass', 'Excellent'),
            ('Good', '5'),
        ),
        (
            GradingSystems.FIVE_POINT,
            ('Not graded', 'Re-credit', '1', '2', '3', '4', '5'),
            ('Fail', 'Pass', 'Good', 'Excellent', '0', '6', '10', '100'),
        ),
        (
            GradingSystems.TEN_POINT,
            ('Not graded', 'Re-credit', '1', '5', '6', '10'),
            ('Fail', '0', '11', '100'),
        ),
        (
            GradingSystems.HUNDRED_POINT,
            ('Not graded', 'Re-credit', '1', '5', '6', '10', '11', '100'),
            ('Fail', '0'),
        ),
    ],
)
@pytest.mark.django_db
def test_gradebook_import_course_grades_different_grading_systems(
    client, grading_system, valid_grades, invalid_grades
):
    teacher = TeacherFactory()
    client.login(teacher)
    course = CourseFactory(teachers=[teacher])
    import_csv_url = reverse(
        'teaching:gradebook_import_course_grades_by_enrollment_id', args=[course.pk]
    )
    enrollment = EnrollmentFactory.create(
        course=course,
        course_program_binding__grading_system_num=grading_system
    )
    valid_lines = [f'{enrollment.id},{grade}' for grade in valid_grades]
    invalid_lines = [f'{enrollment.id},{grade}' for grade in invalid_grades]
    csv_data = force_bytes(
        'id,Final grade'
        + '\n'
        + '\n'.join(valid_lines)
        + '\n'
    ...
    total_lines_count = len(valid_lines) + len(invalid_lines)
    assert (
        f'{valid_lines_count} out of {total_lines_count} rows with valid student IDs.'
        in str(messages[0])
    )
    assert all(
        error_msg in str(messages[1])
        for error_msg in [
            f"Row {i}: Grade '{grade}' doesn't exist or isn't valid for this course's grading system"
            for i, grade in enumerate(invalid_grades, start=valid_lines_count + 1)
        ]
    )
    assert EnrollmentGradeLog.objects.count() == valid_lines_count
