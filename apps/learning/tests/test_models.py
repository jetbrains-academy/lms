...
from courses.models import CourseGroupModes, CourseNews, Semester, StudentGroupTypes, CourseProgramBinding
...
from learning.models import (
    ...
    StudentAssignment, StudentGroup, Enrollment
)
from learning.tests.factories import (
    AssignmentCommentFactory, AssignmentNotificationFactory,
    CourseNewsNotificationFactory, EnrollmentFactory,
...
def test_course_delete():
    course = CourseFactory(group_mode=CourseGroupModes.MANUAL)
    CourseProgramBindingFactory.create_batch(3, course=course)
    StudentGroupFactory.create_batch(3, type=StudentGroupTypes.MANUAL, course=course)
    course.delete()
    assert StudentGroup.objects.count() == 0
    assert CourseProgramBinding.objects.count() == 0

    course = CourseFactory(group_mode=CourseGroupModes.PROGRAM)
    CourseProgramBindingFactory.create_batch(3, course=course)
    assert StudentGroup.objects.filter(course=course).count() == 3
    course.delete()
    assert StudentGroup.objects.count() == 0
    assert CourseProgramBinding.objects.count() == 0

    course = CourseFactory(group_mode=CourseGroupModes.PROGRAM_RUN)
    binding = CourseProgramBindingFactory(course=course)
    EnrollmentFactory(
        student_profile__academic_program_enrollment__program=binding.program,
        student_profile__academic_program_enrollment__start_year=2024,
        course=course,
        course_program_binding=binding,
    )
    EnrollmentFactory(
        student_profile__academic_program_enrollment__program=binding.program,
        student_profile__academic_program_enrollment__start_year=2025,
        course=course,
        course_program_binding=binding,
    )
    EnrollmentFactory(
        student_profile__academic_program_enrollment__program=binding.program,
        student_profile__academic_program_enrollment__start_year=2026,
        course=course,
        course_program_binding=binding,
    )
    assert StudentGroup.objects.filter(course=course).count() == 3
...
@pytest.mark.django_db
def test_course_delete_with_existing_enrollments():
    # We won't allow deleting a course or a course binding if there is at least one enrollment.
    course = CourseFactory(group_mode=CourseGroupModes.MANUAL)
    binding = CourseProgramBindingFactory(course=course)
    StudentGroupFactory.create_batch(1, type=StudentGroupTypes.MANUAL, course=course)
    EnrollmentFactory(
        student_profile__academic_program_enrollment__program=binding.program,
        student_profile__academic_program_enrollment__start_year=2024,
        course=course,
