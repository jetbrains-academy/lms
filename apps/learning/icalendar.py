...
def generate_vtimezone(tz: tzinfo):
    assert tz is not UTC
    assert tz is not ZoneInfo('UTC')
    tzc = Timezone()
    tzc.add('TZID', tz)
    tzc.add('X-LIC-LOCATION', tz)
    std_comp = TimezoneStandard()

    # Getting current time zone offset excluding DST
    dt = datetime.now(tz)
    offset = dt.utcoffset() - dt.dst()
    std_comp.add('TZOFFSETFROM', offset)
    std_comp.add('TZOFFSETTO', offset)

    std_comp.add('TZNAME', str(tz))
...
class ICalendarEvent(ABC):
    ...
    def __init__(self, time_zone: tzinfo,
...
class StudyEventICalendarEvent(ICalendarEvent):
    ...
    def _model_to_dict(self, instance: Event):
        ...
        return {
            ...
            'summary': vText(instance.name),
            'description': vText(description),
            'dtstart': starts_at.replace(tzinfo=self.time_zone),
            'dtend': ends_at.replace(tzinfo=self.time_zone),
            'created': instance.created,
            'last-modified': instance.modified,
