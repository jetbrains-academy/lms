...
class ICalClassesView(UserICalendarView):
    ...
    def get_calendar_meta(user, site, url_builder, tz) -> ICalendarMeta:
        return ICalendarMeta(
            name=f"Classes {site.name}",
            description=f"{site.name} class schedule ({user.get_full_name()})",
...
class ICalAssignmentsView(UserICalendarView):
    @staticmethod
    def get_calendar_meta(user, site, url_builder, tz) -> ICalendarMeta:
        description = "Assignment Due Dates {} ({})".format(
            site.name, user.get_full_name())
        return ICalendarMeta(
            name=f"Assignments {site.name}",
...
class ICalEventsView(UserICalendarView):
    ...
    def get_calendar_meta(user, site, url_builder, tz) -> ICalendarMeta:
        return ICalendarMeta(
            name=f"Events {site.name}",
            description="General Events Calendar {}".format(site.name),
