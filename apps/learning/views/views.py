...
class AssignmentSubmissionBaseView(StudentAssignmentURLParamsMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        context = {
            ...
            'get_comment_element_class': self.get_comment_element_class,
        }
        return context

    @staticmethod
    def get_comment_element_class(comment: AssignmentComment):
        if comment.author is None:
            return 'author-system'
        elif comment.author == comment.student_assignment.student:
...
class AssignmentAttachmentDownloadView(ProtectedFileDownloadView):
    ...
    def get_protected_object(self) -> Optional[AssignmentAttachment]:
        ids = sqids.decode(self.kwargs['sid'])
        if not ids:
            raise Http404
