...
from learning.models import (
    AssignmentGroup, Enrollment, StudentAssignment, StudentGroup
)
...
class EnrollOrLeavePermissionObject(NamedTuple):
    course: Course
    student_profile: StudentProfile


def can_enroll_or_leave(student_profile: StudentProfile | None, course: Course) -> bool:
    if not student_profile or course.is_completed:
        return False
    ...
    elif student_profile.academic_program_enrollment:
        return (
            Course.objects
            .filter(id=course.id)
            .student_can_enroll_from_program(student_profile)
            .exists()
        )
...
@rules.predicate
def enroll_in_course(user, permission_object: EnrollOrLeavePermissionObject):
    ...
    if not student_profile.is_active:
        logger.debug("Permissions for student with inactive profile "
                     "are restricted")
        return False
    if not can_enroll_or_leave(student_profile, course):
        return False
    return True
...
def learner_has_access_to_the_assignment(user: User,
    ...
    if not enrollment:
        return False
    if timezone.now() < student_assignment.assignment.opens_at:
        return False
...
class ViewCourseReviews(Permission):
    ...
    @rules.predicate
    def rule(user, course: Course):
        return not course.is_completed
...
class LeaveCourse(Permission):
    ...
    @staticmethod
    @rules.predicate
    def rule(user, permission_object: EnrollOrLeavePermissionObject):
        if not user.get_enrollment(permission_object.course):
            return False
        return can_enroll_or_leave(
            student_profile=permission_object.student_profile,
            course=permission_object.course,
        )
