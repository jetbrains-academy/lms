...
def create_assignment_comment(*, personal_assignment: StudentAssignment,
                              is_draft: bool, created_by: User | None,
                              message: Optional[str] = None,
                              attachment: UploadedFile | Literal[False] | None = None,
    ...
    # ClearableFileInput value is False when file is cleared and None when file is unchanged
...
def _get_draft_submission(user: User | None,
...
def get_draft_comment(user: User | None, student_assignment: StudentAssignment):
...
def update_personal_assignment_score(*, student_assignment: StudentAssignment,
                                     changed_by: User | None, source: AssignmentScoreUpdateSource,
...
def create_personal_assignment_review(
    *,
    ...
    jba_solved_task_ids: Optional[list[int]] = None
) -> AssignmentComment:
    """
    ...
    if jba_solved_task_ids is not None:
        meta['jba_solved_task_ids'] = jba_solved_task_ids
    if not is_draft:
...
def get_assignment_update_history_message(comment: AssignmentComment) -> str:
    ...
    is_status_changed = status_new and status_old != status_new
    if score_new is None:
        score_new = "Not graded"
    status_label = AssignmentStatus(status_new).label if status_new else None
    text = ''
    if is_score_changed and is_status_changed:
        text = f"New grade assigned: <code>{score_new}</code>, and status updated to {status_label}."
    elif is_score_changed:
        text = f"New grade assigned: <code>{score_new}</code>"
    elif is_status_changed:
        text = f"Status updated to {status_label}"
    # mark safe
    return Markup(text)
